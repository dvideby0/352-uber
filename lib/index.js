...
function Uber(data) {
  ...
  if (_.isString(data)) {
    ...
    var  baseUrl;
    if (data.use_sandbox) {
      baseUrl = config.sandboxUrl + api_version;
    } else {
      baseUrl = config.baseUrl + api_version;
    }
  }
  ...
  function makeRequest(params, callback){
    ...
    var url = baseUrl + endpoint.request + formatParams(params);
    helper.get(this.getAuthToken(), url, callback);
  }
